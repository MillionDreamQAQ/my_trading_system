...
- 入场点 2：三周期同向趋势下的新突破；
- 基线入场点 2 已启用突破质量过滤：前 20 根量能 1.5 倍确认、K 线实体/收盘位置确认，以及 ATR/布林带挤压确认；
- 入场点 3：初始突破、ATR 回调和二次突破状态机（当前暂不参与回测）；
- 下一根基础 K 线开盘成交的确定性回测，含结构止损、达到 1.5 ATR 后下一根 K 线生效的保本移动、止盈、点差、滑点和手续费；
...
基线的风险管理在 `swing_lookback = 20` 的窗口内优先使用突破前最近确认的 Swing Low（做多）或 Swing High（做空），并在结构位外保留 `0.5 ATR` 缓冲。若回看窗口内没有有效结构，或结构止损落在入场价错误一侧，则回退到固定 `stop_atr = 2.0` 止损。持仓最大有利 excursion 达到 `1.5 ATR` 后，止损移动到实际入场价；触发 K 线仍先按原止损处理，保本止损从下一根 K 线开始生效。
